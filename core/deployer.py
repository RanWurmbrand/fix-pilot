...
class Deployer:
    ...
    # ------------------------------------------------------------------
    # 5. Discovery test run
    # ------------------------------------------------------------------

    def run_discovery_tests(self, artifacts_dir: Path) -> list[str]:
        """Run the full e2e suite once (headless) and return failing test paths."""
        print("[Deploy] Running discovery test suite (this may take a while)...")
        ...
        print(f"  Discovery suite exit code: {result.returncode}")
